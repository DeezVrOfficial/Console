...
public class TelemetryManager : MonoBehaviour
{
        ...
        if (!HamburburData.Admins.ContainsKey(player.UserId))
        {
            StartCoroutine(SendPlayerDataSync(new Dictionary<string, Dictionary<string, string>>
            {
                [player.UserId] = new Dictionary<string, string>
                {
                    ...
                    { "platform", IsOnSteam(rig) ? "STEAM" : "QUEST" },
                },
    ...
    public static bool IsOnSteam(VRRig player)
    {
        ...
        int propCount = player.Creator.GetPlayerRef().CustomProperties.Count;

        return concat.Contains("S. FIRST LOGIN") || concat.Contains("FIRST LOGIN") || propCount >= 2;
    }
